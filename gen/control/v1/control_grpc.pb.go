...
const (
	AgentControl_Connect_FullMethodName = "/flux.control.v1.AgentControl/Connect"
)
...
type AgentControlClient interface {
	Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentEvent, ControllerEvent], error)
}
...
func (c *agentControlClient) Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentEvent, ControllerEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentControl_ServiceDesc.Streams[0], AgentControl_Connect_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[AgentEvent, ControllerEvent]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentControl_ConnectClient = grpc.BidiStreamingClient[AgentEvent, ControllerEvent]
...
type AgentControlServer interface {
	Connect(grpc.BidiStreamingServer[AgentEvent, ControllerEvent]) error
...
func (UnimplementedAgentControlServer) Connect(grpc.BidiStreamingServer[AgentEvent, ControllerEvent]) error {
...
func _AgentControl_Connect_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentControlServer).Connect(&grpc.GenericServerStream[AgentEvent, ControllerEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentControl_ConnectServer = grpc.BidiStreamingServer[AgentEvent, ControllerEvent]
...
var AgentControl_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "flux.control.v1.AgentControl",
	...
	Metadata: "flux/control/v1/control.proto",
}
